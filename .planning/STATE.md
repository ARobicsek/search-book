...
| Participant hover (meetings) | Hovering a participant on a meeting card or in the editor shows title + current employer via shared `PersonTooltip`; `/meetings` participant include + `/contacts/names` carry `title` + primary `company.name` | 2026-06-18 |
