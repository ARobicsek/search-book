...
**Phase 2 COMPLETE (2026-06-04).** All of Tasks 15–18, 20–25 are on branch `claude/bold-ride-OjoVC`, each an atomic commit; full client+server typecheck and a client production build pass. Task 25 was resolved by decision+documentation (see below) since photos are already backed up off-platform by the existing manual photo-ZIP.

Brief — each is a small, self-contained task.
...
- **Task 15 — PWA stale-data risk.** `NetworkFirst` 1-day API cache ([client/vite.config.ts:44-55](../client/vite.config.ts)) can feed stale data to auto-save on flaky connections, overwriting newer server values. Exclude mutable record GETs from caching (or `NetworkOnly` for `/api/`), or shorten `maxAgeSeconds` and show an "offline — showing cached data" banner.
  **STATUS: ✅ DONE (commit c433bfa).** `/api/` runtime caching switched from `NetworkFirst` to `NetworkOnly` — API responses are never cached, removing the stale-overwrite vector entirely. Trade-off (no offline record reads) accepted for write correctness. Photo cache (`CacheFirst`) unchanged.
- **Task 16 — Rate limiting + smaller body limit.** Add `express-rate-limit` (global cap) and lower `express.json({ limit })` from `50mb` → ~`2mb` ([app.ts:56](../server/src/app.ts)). Closes brute-force of the password gate, scraping, OpenAI-cost abuse via `/api/linkedin`, and memory-DoS.
  **STATUS: ✅ DONE (commit 565f0dd).** `express-rate-limit` added: general 1000/15min on `/api` (placed before the auth gate so it throttles password brute-forcing; skips `/health`) + strict 40/hour on `/api/linkedin` (OpenAI cost). Global JSON body limit dropped 50mb→2mb; `/api/backup` keeps a 50mb parser (mounted first, so body-parser's `_body` flag makes the global one skip) for full-DB restores. `trust proxy` set for correct client IP behind Vercel. General cap chosen at 1000 (not the suggested ~300) so heavy real browsing — ~10 requests/page — never trips it.
- **Task 17 — Error tracking (Sentry).** Add Sentry free tier to Express + React; wire into the error boundary (Task 11).
  **STATUS: ✅ DONE (commit 89ec4a6).** `@sentry/node` (server) + `@sentry/react` (client), both opt-in via `SENTRY_DSN` / `VITE_SENTRY_DSN` and no-op when unset (local dev unaffected). Server init in `server/src/sentry.ts`, imported before express so HTTP is instrumented; `setupExpressErrorHandler` after routes. Client init in `client/src/lib/sentry.ts` (imported in `main.tsx`); `ErrorBoundary.componentDidCatch` now calls `Sentry.captureException` with the React component stack. `tracesSampleRate: 0` (errors only — stays in free tier). **[USER ACTION] to activate:** create a free Sentry project and set `SENTRY_DSN` (Vercel prod) and `VITE_SENTRY_DSN` (Vercel prod, build-time) — `printf 'DSN' | vercel env add VAR production`.
- **Task 18 — Input allow-listing.** Several write routes spread raw `req.body` into Prisma ([companies.ts:305](../server/src/routes/companies.ts), [relationships.ts:98](../server/src/routes/relationships.ts), others). Destructure only known fields to prevent mass-assignment.
  **STATUS: ✅ DONE (commit 60e4686).** `PUT /companies/:id` now copies only an allow-list (`name, industry, size, website, hqLocation, notes, status`) via a `pickWritable` helper; `PUT /relationships/:id` copies only `type`/`notes` (never the `fromContactId`/`toContactId` identity columns). No more id/createdAt/updatedAt/unknown-key passthrough. (Contact PUT goes through `processFormData`, which already shapes its fields; left as-is.)
- **Task 20 — Guard unguarded `JSON.parse`.** [companies.ts:244,261](../server/src/routes/companies.ts) parse `additionalCompanyIds`/`connectedCompanyIds` with no try/catch — malformed JSON 500s the request. Wrap with a `[]` default, matching the pattern used elsewhere.
  **STATUS: ✅ DONE (commit 46d3b30).** Added a `safeParseArray` helper (parses, falls back to `[]` on bad/ non-array JSON); both unguarded parses in `POST /companies/:id/contacts` now use it.
- **Task 21 — Scrub JSON-array refs on company delete.** Deleting a company leaves dangling IDs in contacts' `additionalCompanyIds`/`connectedCompanyIds` (no FK on JSON strings). Reuse the merge-logic scrub on delete.
  **STATUS: ✅ DONE (commit 70f56ee).** `DELETE /companies/:id` now, in one `$transaction`, finds referencing contacts (substring pre-filter → exact parse-and-filter), removes the deleted id from both JSON arrays (nulling an emptied array), then deletes the company. The primary `companyId` FK was already `SetNull` at the DB.
- **Task 22 — PWA update prompt.** `registerType: 'autoUpdate'` ([vite.config.ts:12](../client/vite.config.ts)) contradicts the existing update-prompt component — old bundles linger after deploys. Switch to `prompt` so [pwa-update-prompt.tsx](../client/src/components/pwa-update-prompt.tsx) works, or force reload on new SW.
  **STATUS: ✅ DONE (commit 853a99c).** `registerType` switched `autoUpdate`→`prompt`; `PWAUpdatePrompt`'s `needRefresh` now fires (it's mounted in `App.tsx`), so the user is offered the new bundle instead of old bundles silently lingering.
- **Task 23 — Reset secondary data on contact switch.** [contact-detail.tsx:267-308](../client/src/pages/contacts/contact-detail.tsx) shows the previous contact's conversations/notes briefly when navigating between contacts. Clear secondary arrays at the top of `loadData` (or key the page on `id`).
  **STATUS: ✅ DONE (commit b05d674).** `loadData` now resets `contact` + all secondary arrays (actions, conversations, relationships, links, prepNotes, employmentHistory, tags, dossier count) and re-enters the loading state before fetching; contact-independent lookup data (allContacts/allCompanies/allTags) is intentionally preserved.
- **Task 24 — Tighten CORS.** [app.ts:36-54](../server/src/app.ts) allows any `*.vercel.app` origin and all no-origin requests. After header-auth (Task 1), restrict to the exact prod domain + localhost.
  **STATUS: ✅ DONE (commit 120195d).** Removed the `*.vercel.app` wildcard; CORS allow-list is now localhost + `https://searchbook-three.vercel.app` (+ optional `VERCEL_URL`/`CLIENT_URL`). No-origin requests (same-origin PWA, curl, uptime monitor, cron) still allowed; dev still allows any origin. Header-auth (Task 1) remains the real gate. Note: previews on other `*.vercel.app` URLs are same-origin (client+API share the URL), so CORS isn't invoked for them.
- **Task 25 — Photo backup (optional).** Backups store only photo URLs, not bytes (images live in Vercel Blob). Lower value than text. Option: have the daily cron also snapshot/zip Blob photos periodically, or accept photos as best-effort and document it.
  **STATUS: ✅ DONE by decision+documentation (2026-06-04).** Chose **"accept best-effort + document"** over building a cron-side photo zip, because: (a) photo *bytes* are **already** backed up off-platform — Settings → "Back up photos" runs `client/src/lib/photo-backup.ts::buildPhotosZip`, which fetches every `photoUrl`/`photoFile` (Blob in prod) and packs them + a `manifest.json` into `searchbook-photos.zip` for the user to keep in OneDrive (the weekly habit); and (b) a cron-side zip would write into the *same* Vercel Blob store as the photos themselves — so it adds no independence (if Blob is lost, both copies are lost) while risking the Vercel 30s timeout on large photo sets. The daily JSON cron therefore intentionally stays photo-free (small/fast). **Posture:** photos = recoverable via the weekly manual ZIP; best-effort for the automatic layer; external avatar URLs that CORS-block are skipped (already reported in the ZIP summary). If true automatic photo redundancy is ever wanted, the right move is shipping the ZIP to an *independent* store (e.g. the OneDrive `backups/` folder), not duplicating into Blob.
