...
**STATUS:** ✅ Code complete 2026-06-12 (commit `b91376a`). Quick Log opens from a header button (all pages), the command palette, and the Meetings page; who-pickers sit behind a collapsed "Who was there" disclosure. New shared `TitleAutocomplete` component. Verified in-browser. **Redesigned 2026-06-15 (`cb5d604`): "promote the big 3"** — Participants/Notes/Follow-up actions are always visible; secondary fields moved into 3 labeled disclosures (Organizations & attendees · Summary & next steps · Tags, prep notes & attachments). Added a **Series picker** (pick existing / create inline) wired into autosave. **Bug fix 2026-06-24 (`8cbc7ee`):** closing the dialog (×/Esc/Cancel) now **flushes** unsaved work instead of discarding it (incl. brand-new free-text participants autosave skips), and the contact/org/tag lookups **refetch on every open** (were cached for the whole session, hiding mid-session-created/merged contacts from the pickers + `@`-autocomplete). **Participant UX 2026-06-29 (`ce9f306`):** adding a participant now **creates the Contact immediately** (typed-in free text or pasted), so it has an id the moment it's added — participant names in the editor are **links to the contact card**; **bulk paste** an Outlook recipient list (`Name <email>; …`) into the field (new `POST /api/contacts/resolve-participants` matches by email→name, creates the rest); and a contact created here that's **removed again before gaining other info is auto-deleted**.
...
**STATUS:** ✅ Code complete 2026-06-12 (commit `b507f0e`). Implemented as a dedicated `GET /api/meetings` route (pagination envelope; series title matched case-insensitively exact; extra `id` param for single-meeting deep links from search). Meetings nav item added to the sidebar. Verified in-browser incl. series view. **Reworked 2026-06-15 (`ccdafdf`/`cb5d604`/`7e6e8a0`):** series view now driven by the real `Series` entity (`?seriesId=`), Title filter → **Series** dropdown, **Sort** dropdown (default recently-updated), series **rename/delete** on the header, card title = first participant entered (new `ConversationParticipant.ordering`), person-name search. ⚠ **Amended 2026-08-06 (`9f9578e`): the default sort is no longer "recently updated" — it is `date:desc`, with any IN-PROGRESS meeting pinned above it.** The owner went looking for the card of the meeting they were sitting in and couldn't find it: `updatedAt` ordering had put it pages below meetings from days earlier. Under date-desc alone a live meeting still sorts below every future meeting *and* every later meeting today (13th down in the local snapshot), so the pin is computed **server-side** in `GET /api/meetings` — the client only holds the pages it has loaded. "Recently updated" remains an option in the dropdown.
...
**STATUS:** ✅ DEPLOYED 2026-06-12 (commits `e099388`…`d718ffa`). Turso CREATE TABLEs run by the user in the console; verified locally in-browser; live deploy + health confirmed. (One Vercel build failure fixed in `d718ffa` — attributed at the time to the client **build** running `tsc -b` with `noUnusedLocals` while the `typecheck` script was "less strict". ⚠ **The real cause, found 2026-08-04:** `typecheck` was `tsc --noEmit` against a *solution* tsconfig (`"files": []`), so it checked **zero files** and could not fail at all. Both now run `tsc -b`, so a build-only failure of this shape shouldn't recur.)
...
**FOLLOW-UPS SHIPPED 2026-08-05 (commit `4839e35`, schema-free)** — four owner-reported picker issues, none of them in the ICS layer: (1) the dialog **opens blank and searches nothing** until a range is named (it used to restore the last-used range from `localStorage` and immediately fetch it — the wrong days, one slow ICS fetch, every open); (2) **non-meeting calendar blocks are filtered** — new `server/src/lib/calendar-filter.ts` flags lunch / "Private Appointment" / travel time / flights / hotels as `excluded` on `GET /calendar/events`, hidden behind a "N skipped — Show" disclosure and out of the default selection **and** "Select all", but still tickable because a first-word rule can catch a real meeting ("Hotel Industry Roundtable"); (3) the **import shows what it is doing** — the body reads "Importing N meetings…" and the range controls disable while it runs; (4) a **successful import closes the dialog** (a failed one stays open with the selection intact). `POST /calendar/import` deliberately does **not** re-apply the filter — what it receives is what the owner explicitly ticked. Verified with a latency-controlled `fetch` stub over the real component (0 requests on open, one per range action, only the real meetings sent, dialog closed after the POST and still open after a 502) plus a 390 px pass; the live ICS feed could not be exercised because Microsoft's endpoint 500s from this network — **so the owner's same-day confirmation ("I confirm that they work well") is the only test against the real calendar, and the only time the subject rules met real subjects.**
