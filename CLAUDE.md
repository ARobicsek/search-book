...
- **Pre-push check**: `npm run prepush` (⚠ the client's `typecheck` must stay `tsc -b --force` — `client/tsconfig.json` is a *solution* file with `"files": []`, so a plain `tsc --noEmit` there follows no references and checks **zero files** while exiting 0. It silently did exactly that until 2026-08-04.)
...
- **The Outlook import picker opens BLANK and searches nothing until a range is named** — `components/import-outlook-dialog.tsx`. It remembers no range (the `outlook_import_range` localStorage key is dead — nothing reads it), so opening it costs **zero** `/calendar/events` requests: the load effect is gated on both dates being set, and an effect on **both edges of `open`** clears preset/range/events/selection so a reopen can neither flash the last session's list nor re-run its search. Choosing a preset or completing both custom dates is what triggers the one request. Two more behaviours the owner asked for, 2026-08-05: **importing takes over the body** with a "Importing N meetings…" spinner and disables the range controls (the button's small spinner alone read as "nothing is happening" on a multi-second ICS-refetch + write), and a **successful import closes the dialog** — a failed one stays open with the selection intact to retry. The subject filter below is what keeps lunch/travel out of the list.
  - Historical, and still the rule for anything that *does* persist: **restore persisted UI state in the `useState` initializer, never an on-open effect** — this picker restored its last-used range in an on-open effect, but the dialog is mounted for the page's whole life, so the first render still held the `next7` default and the load effect — *same commit, state updates not yet applied* — fetched **that** range before the restore landed. Every open ran two overlapping `/calendar/events` requests, and nothing decided which response was still wanted: picking "Today" painted 7 days, then tomorrow, then today; and when the stale wide request happened to be the slowest it **won**, leaving a week of meetings on screen with Today selected and `07/31 → 07/31` in the date inputs — importing from there would have created the wrong meetings, so this class of bug is not merely cosmetic. Two rules: (1) read `localStorage` in the **`useState` initializer** — an on-open effect may only *re-resolve* presets against today's date (so a tab left open overnight can't reopen on yesterday's "Today") and must hand back the **same object** when nothing moved, or it re-triggers the fetch (moot in *this* file now that it persists nothing — the on-open effect here only ever writes the blank state, which the load gate refuses to fetch); (2) `load()` stamps a **sequence number** and aborts the previous `AbortController`, so a superseded response can't set events, report an error, or clear the spinner — `finally` runs after an early `return`, so the guard belongs in there too. Same discipline as global search above. ⚠ **Measure a symptom whose appearance depends on timing** — which response wins is a race, so this was reproduced with a **latency-controlled `fetch` stub** and counted (pre-fix 3 requests / 4 paints, and the wrong final state when the stale one was slowest; post-fix one request per user action, one paint, correct in both orderings) rather than reasoned about.
- **The meetings list puts an IN-PROGRESS meeting FIRST, then sorts by date/time descending** —
  `pages/meetings.tsx` + `routes/meetings.ts`. The default sort was `updatedAt` ("Recently updated")
  until 2026-08-06: the owner, hunting for the card of the meeting they were sitting in, found it
  pages below meetings from days earlier. Nothing was broken — the list simply wasn't ordered by
  date, and the sort menu is the only place that said so. Default is now `date:desc` (ties broken by
  `startTime`); "Recently updated" is still an option. ⚠ **The "now" pin has to be SERVER-side.**
  Under date-desc a live meeting sorts below every *future* meeting **and** every later meeting today
  — 13th down in the local snapshot, further with a week of Outlook imports loaded — so hoisting it
  client-side would miss it exactly when it lands past page 1. `GET /meetings` therefore takes the
  client's Eastern `today`/`now` (now sent on **every** request, not just the past/upcoming modes),
  pulls today's already-started meetings under the *same* filters, confirms each with a mirror of
  `isHappeningNow`, and pins them ahead of the sort; the page window is split between the pinned rows
  and a rest-query that excludes them, so `total`, "Show more", and offsets stay exact. The client
  re-partitions the loaded list too, so the order follows the 30s clock tick without a refetch — and
  `loadMore` dedupes by id, because a meeting that *ends* between page 1 and page 2 shifts the window
  by one row. Verified against the real endpoint (pin position, both assumed-duration edges, 4-page
  pagination for dupes/gaps, and that `onlyUpcoming` / a date range still bound the pin).

- **The Mentions page is ONE feed, and "when logged" means two different fields** — `pages/mentions.tsx`
  + `mentionFeed()` in `lib/mentions.ts`. It used to render two stacked lists (all contact notes / ideas,
  then all meetings behind a "Load more"), so a note written today and a meeting held today sat pages
  apart. They are now one reverse-chronological stream: newest day first, newest time first within the day.
  ⚠ **The two kinds don't carry the same sort field, and they aren't the same kind of value.** A meeting
  sorts by its own `date` + `startTime` (Eastern *wall clock*); a note source sorts by `updatedAt`, which
  is an **instant** — so it is projected into Eastern (`easternPartsOfTimestamp`) before the two are
  compared, or a note logged at 9 PM ET sorts into the next day. That helper also normalizes the zoneless
  `YYYY-MM-DD HH:MM:SS` form a backup restore / raw-SQL write leaves behind, which `new Date()` would
  otherwise read as *local*. An item with no time sorts **last within its day**, matching the meetings
  list's null-`startTime` convention (SQLite ranks NULL smallest). Both card kinds show the date and time
  that placed them — without that the order looks arbitrary. `GET /mentions` breaks its date tie with
  `startTime` because the client merges its **pages** with the note sources and re-sorts, which only holds
  if each page is itself in order. Two honest limits: `Contact.updatedAt` is the *record's* last-touched
  time, not when the note text was written (editing any field floats that card up), and because note
  sources all load at once while meetings paginate, "Load more" can insert older meetings above the older
  notes tail — the list is correctly ordered at every point, but it is not append-only.
- **The "meeting is happening now" rule lives in ONE place** — `lib/meeting-time.ts` (`easternNowParts`,
  `easternParts`, `isHappeningNow`, `addMinutes`, `ASSUMED_MEETING_MINUTES`) plus `hooks/use-clock-tick.ts`.
  It was private to `meetings.tsx` until the Mentions feed needed the same green left border and pulsing
  "Now" badge; it was **moved, not copied**, so the two pages can't drift on what "now" means. Any new
  surface that flags a live meeting imports these — and must also call `useClockTick()`, or the marker
  never turns itself off. The one **deliberate** copy is server-side (`isHappeningNow` in
  `routes/meetings.ts`, for the in-progress pin above) — the server can't import client code, so it
  mirrors the rule the same way `notUpcomingClause` mirrors `isUpcomingMeeting`. Change one, change both.
...
- **@-mentions live in THREE sources, across TWO index tables.** A meeting's notes/nextSteps/prep notes → `ConversationMention`. A **contact's `notes`** and an **idea's `description`** → `NoteMention` (`syncNoteMentions`, same delete-all-and-re-derive contract, same FK-safe degradation of unknown ids to loose mentions). ⚠ Two tables rather than nullable columns on one because making `ConversationMention.conversationId` nullable is a **SQLite table rebuild**, which the additive-DDL-only Turso path can't take; a new table is a plain `CREATE TABLE`. `NoteMention` sets exactly one of `sourceContactId`/`sourceIdeaId` (both `onDelete: Cascade`) and **derives** `sourceType` from which one is set rather than storing it, so the discriminator can't drift. Notes is the ONLY field on the contact form with the picker (the narrative box is where a third party actually gets brought up). Every review surface covers both indexes: the Mentions page (**one merged feed**, see below; `MentionResolveRow` is shared, parameterised by `basePath` = `/mentions` vs `/mentions/note`), the "Mentioned In" cards on contact/org pages, `GET /mentions/index` counts, and the search `mentions` scope (which returns `noteMentions` beside `mentions`, summed into one `totals.mentions`). **A new mention-bearing field must be wired into all of them, plus the merge rewrite below.**
- **Resolving a loose mention is find-or-**create**, and merges must rewrite its token.** A loose token (`#mention` / `#org-mention`) is inert text: nothing retro-binds it when a matching record is created later, so the Mentions page keeps offering "Create" for a name that already exists. `POST /mentions/:id/create-company` used to call `company.create` blind, which is how *Peterson Center on Healthcare* and *Battelle* each ended up with two rows (the fingerprint: the duplicate has **no `CompanyStatusHistory`** — only `POST /companies` and `/companies/resolve` write one). Both create-* routes now go through `resolveExistingCompanyByName` / `resolveExistingContactByName` and return `linked: true` when they bind instead of create; `POST /mentions/:id/link` binds to a record the owner picks (for near-misses like note "Peterson Health" → org "Peterson Center on Healthcare" — the prose wording is preserved, only the token target changes). ⚠ Symmetrically, **any merge that deletes a record must rewrite `(/companies/N)` / `(/contacts/N)` in note text and re-sync** (`runCompanyMerge`/`runContactMerge` in `routes/duplicates.ts`): `ConversationMention.companyId` is `onDelete: SetNull`, so skipping it leaves a dead link in the prose that degrades back to a *loose* mention on the next save — re-offering "Create" and re-minting the duplicate just merged away. That rewrite covers **all five mention-bearing fields**: `Conversation.notes`/`nextSteps`, `ConversationPrepNote.content`, and — via `rewriteNoteMentionTokens` — `Contact.notes` and `Idea.description`. It must run **before** the delete, because after it nothing points at `removeId` to say which sources need re-syncing. `runContactMerge` additionally re-derives the **kept** contact's index unconditionally, since field selections can replace its whole `notes` value.
...
- **Non-meeting calendar blocks are FLAGGED, never dropped** — `server/src/lib/calendar-filter.ts`. Lunch holds, Outlook's own "Private Appointment" placeholder and travel logistics aren't meetings, so `GET /calendar/events` marks them `excluded: true` (whole-subject match on *lunch* / *private appointment* / *travel time*; first-word match on *flight(s)* / *hotel(s)*). The picker hides them behind an "N lunch/travel/private blocks skipped — Show" disclosure, keeps them out of the default selection **and** out of "Select all", but leaves the checkbox live: a first-word rule can catch a real meeting ("Hotel Industry Roundtable"), and a row the owner has to tick by hand beats one that silently vanished. That opt-in is why `POST /calendar/import` deliberately does **not** re-check the filter — what it receives is what was explicitly ticked. Add a rule here, not in the client.
