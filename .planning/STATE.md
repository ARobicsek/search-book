...
| Meeting @-mentions | Type `@` in meeting notes/next-steps to flag a third person inline — existing contact **or** a "loose" name not yet in the CRM. Tokens live in the note text (source of truth): `[@Name](/contacts/ID)` resolved / `[@Name](#mention)` loose. New `ConversationMention` table (`contactId` nullable) is **derived** from notes on every conversation save (delete-all + re-create). Review on `/mentions` (global + `?contactId`) and a "Mentioned in Meetings" contact card; loose mentions get one-click "Create contact" (`POST /api/mentions/:id/create-contact` rewrites the token). `@` autocomplete is opt-in on `MarkdownTextarea` (`mentionContacts`); chips render via `MentionableMarkdown`. Backup version 6 | 2026-06-18 |
