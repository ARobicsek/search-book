...
// GET /api/contacts/:id/full — single contact with ALL nested details (prevents Vercel connection exhaustion)
router.get('/:id/full', async (req: Request, res: Response) => {
  try {
    ...
    const [
      ...
      prisma.action.findMany({
        ...
        include: {
          ...
          company: { select: { id: true, name: true } },
          actionContacts: { include: { contact: { select: { id: true, name: true } } } },
          actionCompanies: { include: { company: { select: { id: true, name: true } } } },
          links: true,
        },
        orderBy: [
          { completed: 'asc' },
      ...
      prisma.relationship.findMany({
        where: { OR: [{ fromContactId: contactId }, { toContactId: contactId }] },
        include: {
      ...
      // 8. Tags via intermediate table
      prisma.contactTag.findMany({
        where: { contactId },
        include: { tag: true },
      }).then(tags => tags.map(ct => ct.tag)),
    ]);
    ...
    res.status(500).json({ error: 'Failed to fetch full contact details' });
  }
});
