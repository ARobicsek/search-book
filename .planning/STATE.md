...
| "Who owes it" model (C1) | Additive `Action.owedByMe` (bool) + `Action.owerContactIds` (JSON); `direction` enum kept **derived** (`owedByMe && owers empty ? OWED_BY_ME : OWED_TO_ME`) so dashboard/analytics are unchanged. *(Confirmed; Task 3 not yet built.)* | 2026-06-14 |
